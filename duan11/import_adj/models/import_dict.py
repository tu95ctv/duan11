...
#                     ('start_width', {'required':True, 'for_excel_readonly':True, 'func': lambda v,n,self: n['vof_dict']['id_char']['val'].startswith(app) and ',' not in n['vof_dict']['modules']['val']}),
            ...
            u'action_import': {
                ...
                'string':'TVCV da co chua',
                'fields' : [
                    ('id_char', {'xl_title':['External ID', u'id'], 'required':True, 'key':True }),
                    ('start_width', {'required':True, 'for_excel_readonly':True, 'func': start_width_}),
                    ('name', {'xl_title':['Action Name', u'name'], 'required':False, 'key':True }),
                    ('type', {'xl_title':[u'type', u'Action Type'], 'required':False, 'key':True }),
                    ('usage', {'xl_title':[u'usage',u'Action Usage'], 'required':False, 'key':True , 'allow_not_match_xl_title':True }),
                    ('auto_search', {'xl_title':[u'auto_search',u'Auto Search'], 'required':False, 'key':True,  'allow_not_match_xl_title':True  }),
                    ('context', {'xl_title':[u'context','Context Value'], 'required':False, 'key':True }),
                    ('res_model', {'xl_title':[u'res_model','Destination Model'], 'required':False, 'key':True }),
                    ('domain', {'xl_title': [u'domain','Domain Value'], 'required':False, 'key':True }),
                    ('filter', {'xl_title':[u'filter',], 'required':False, 'key':True, 'allow_not_match_xl_title':True  }),
                    ('limit', {'xl_title':[u'limit',], 'required':False, 'key':True , 'allow_not_match_xl_title':True }),
                    ('res_id', {'xl_title': [u'res_id','Record ID'], 'required':False, 'key':True, 'allow_not_match_xl_title':True}),
                    ('multi', {'xl_title':[u'multi','Restrict to lists'], 'required':False, 'key':True , 'allow_not_match_xl_title':True }),
                    ('src_model', {'xl_title':[u'src_model','Source Model'], 'required':False, 'key':True }),
#                     ('multi', {'xl_title': [u'multi',], 'required':False, 'key':False }),
                    ('target', {'xl_title': [u'target','Target Window'], 'required':False, 'key':True }),
                    ('view_mode', {'xl_title': [u'view_mode','View Mode'], 'required':False, 'key':True }),
                    ('view_type', {'xl_title':[u'view_type','View Type'], 'required':False, 'key':True }),
                    ('view_id', {'xl_title':[u'view_id/id','View Ref.'], 'required':False, 'write_false':True ,'func': lambda v,n,self: '' if not v else v }),
                    ('search_view_id', {'xl_title':[u'search_view_id/id','Search View Ref.'], 'required':False, 'write_false':True, 'func': lambda v,n,self: '' if not v else v }),
                    ('id_number', {'xl_title':[u'ID',], 'required':False}),
                      ]
                },#End stock.inventory.line'   
